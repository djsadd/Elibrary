  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        const data = await api<Subject[]>(`/api/catalog/subjects/details${params.size ? `?${params.toString()}` : ""}`);
    ...
    <div className="space-y-4">
      <form
        onSubmit={async (e) => {
          ...
          try {
            setCreating(true);
            await api("/api/catalog/subjects", {
              method: "POST",
      ...
      >
        <div className="flex-1">
          <label className="block text-sm text-slate-600 mb-1">Add Subject</label>
          <input
            value={newName}
      ...
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <div className="text-slate-500 text-sm">Browse and search subjects</div>
        </div>
      ...
      <div className="bg-white border rounded-md">
        {loading ? (
          ...
          <div>
            {grouped.length === 0 ? (
              <div className="p-6 text-slate-500">No subjects found.</div>
            ) : (
              grouped.map(([letter, names]) => (
                <div key={letter}>
                  ...
                  <ul className="divide-y">
                    {names.map((subject) => (
                      <li key={String(subject.id)} className="px-4 py-3 flex items-center gap-3 hover:bg-slate-50">
                        <Avatar name={subject.name} />
                        <div className="flex-1 min-w-0">
                          <div className="text-sm text-slate-800 truncate">{subject.name}</div>
                        </div>
                        <Link
